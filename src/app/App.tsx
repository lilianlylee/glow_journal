  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-pink-400 mt-1">Capture the magic of every day 🌙</p>
      </div>
          ...
          {cells.map((day, i) => {
            ...
            return (
              <button
                ...
                disabled={isFuture}
                className={[
                  "aspect-square rounded-xl flex flex-col items-center justify-center gap-0.5 text-sm transition-all",
                  isToday ? "bg-pink-400 text-white font-semibold shadow-sm" : "",
                  !isToday && entry ? "bg-pink-50 text-rose-700 hover:bg-pink-100" : "",
                  !isToday && !entry && !isFuture ? "text-rose-700 hover:bg-pink-50" : "",
                  isFuture ? "text-pink-200 cursor-not-allowed" : "cursor-pointer",
                ].join(" ")}
              >
                <span className="text-xs">{day}</span>
                {entry && !isToday && <span className="text-[10px] leading-none">{MOOD_META[entry.mood].emoji}</span>}
              </button>
            );
          })}
        </div>
